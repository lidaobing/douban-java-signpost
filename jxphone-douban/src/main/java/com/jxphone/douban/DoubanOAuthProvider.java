...
public class DoubanOAuthProvider extends DefaultOAuthProvider {
	...
	public DoubanOAuthProvider() {
		this("http://www.douban.com/service/auth/request_token",
	...
	protected void retrieveToken(OAuthConsumer consumer, String endpointUrl,
        ...
        try {
        	...
        	query.put(OAuth.OAUTH_CONSUMER_KEY, consumer.getConsumerKey());
        	query.put(OAuth.OAUTH_SIGNATURE_METHOD, "HMAC-SHA1");
        	query.put(OAuth.OAUTH_TIMESTAMP, Long.toString(System.currentTimeMillis() / 1000L));
        	query.put(OAuth.OAUTH_NONCE, Long.toString(new Random().nextLong()));
        	if(additionalParameters != null 
        			&& additionalParameters.length > 0 
        			&& additionalParameters[0].equals(OAuth.OAUTH_CALLBACK)) {
        		// no-op
        	} else {
        		query.put(OAuth.OAUTH_TOKEN, consumer.getToken());
        	}
        	query.put(OAuth.OAUTH_SIGNATURE, getSignature(consumer.getConsumerKey(),
        	...
        	for(Entry<String, String> entry: query.entrySet()) {
        		...
        		sb.append(OAuth.percentEncode(entry.getKey()))
        			.append("=")
        			.append(OAuth.percentEncode(entry.getValue()));
        	}
	...
	private String getSignature(String consumerKey, String consumerSecret, String tokenSecret, String method, String url, Map<String, String> query) throws OAuthMessageSignerException {
        try {
            String keyString = OAuth.percentEncode(consumerSecret) + '&'
	...
	private String getSignatureBaseString(String method, String url, Map<String, String> query) throws OAuthMessageSignerException {
        try {
	...
	private String normalizeRequestParameters(Map<String, String> query) {
        ...
        boolean first = true;
        for (String key: keys) {
            if (OAuth.OAUTH_SIGNATURE.equals(key) || "realm".equals(key)) {
                continue;
            }
            ...
            first = false;
            sb.append(OAuth.percentEncode(key)).append('=').append(OAuth.percentEncode(query.get(key)));
        }
        return sb.toString();		
	}
